...
interface Submission {
  ...
  timestamp: Date;
  faceId?: string; // Unique identifier for the face scan
  isFirstTimeUser: boolean; // Flag to check if this is user's first submission
}
const submissions: Submission[] = [];
...
interface SuccessSubmission {
    ...
    selectedDrinks?: string[];
    faceId?: string;
    isFirstTimeUser: boolean; // Add this to track first-time users
}
...
export async function submitPhoneNumber(prevState: SubmitResult | null, formData: FormData): Promise<SubmitResult> {
  ...
  // Check if this is a first-time user (both face and phone are new)
  const isFirstTimeUser = !successfulSubmissions.has(cleanPhoneNumber) && !usedFaces.has(faceId);
  ...
  try {
    const newSubmission: Submission = {
      ...
      timestamp: new Date(),
      faceId: faceId,
      isFirstTimeUser: isFirstTimeUser, // Flag if this is user's first time
    };
    ...
    const successData: SuccessSubmission = {
      ...
      selectedDrinks: newSubmission.selectedDrinks.length > 0 ? newSubmission.selectedDrinks : undefined,
      faceId: newSubmission.faceId,
      isFirstTimeUser: newSubmission.isFirstTimeUser
    };

    // Customize message based on whether it's a first-time user
    const message = isFirstTimeUser 
      ? "Welcome! Your first selection has been recorded."
